...
export default function ChatbotScreen() {
  ...
  // Browser microphone / speech-to-text support (Urdu)
  useEffect(() => {
  ...
  const toggleVoice = () => {
    ...
    if (!recognition) {
      alert('آپ کے براؤزر میں وائس ان پٹ سپورٹ نہیں ہے۔ Chrome یا Edge استعمال کریں۔');
      return;
    }
  ...
  const speakReply = (msg) => {
    if (!('speechSynthesis' in window)) {
      alert('آپ کے براؤزر میں وائس آؤٹ پٹ سپورٹ نہیں ہے۔');
      return;
    }
    ...
    <div className="flex flex-col h-[calc(100vh-60px)] max-w-[680px] mx-auto" dir="rtl">
            ...
            <div className="max-w-[72%]">
              ...
              {msg.role === 'bot' && (
                <button
                  ...
                  aria-label="جواب سنیں"
                  title={speakingId === msg.id ? 'آواز بند کریں' : 'جواب سنیں'}
                  className="mt-1 mr-1 text-green-600 hover:text-green-800 transition-colors"
                >
      ...
      {listening && (
        <div className="mx-4 mb-2 px-4.5 py-3.5 bg-red-50 border-[1.5px] border-red-200 rounded-2xl flex items-center justify-between">
          <span className="urdu-sm text-red-600 text-sm">سن رہا ہوں... اردو میں بولیں، پھر مائیک دوبارہ دبائیں</span>
      ...
      <div className="px-4 pt-3 pb-4 bg-white border-t border-slate-100 flex gap-2.5 items-center">
        ...
        >
          {listening ? <MicOff size={21} strokeWidth={2.5} /> : <Mic size={21} strokeWidth={2.5} />}
        </button>
